...
class CommonUtility{
    ...
    int MaximumWidthOfBinaryTree(node *root){
        ...
        while(not process.empty()){
            auto tuple = process.front();
            int level_size = process.size();
            int level_start = process.front().second;
            int level_end = process.back().second;
            max_width = max(max_width,level_end-level_start+1);
            for(int i =0;i<level_size;i++){
              auto tuple = process.front();
              process.pop();
              node *curr = tuple.first;
              int indx = tuple.second - level_start;
              if(curr->left) process.push({curr->left,(2*indx+1)});
              if(curr->right) process.push({curr->right,(2*indx+2)});
            }
        }
        return max_width;
    }
    ...
    int kth_smallest = -1;
    void KthSmallestInBSTUtil(node *root,int k){
        if(root==NULL) return;
        KthSmallestInBSTUtil(root->left,k);
        smallest_count+=1;
        if(smallest_count==k){
            kth_smallest = root->val;
            return;
        }
        KthSmallestInBSTUtil(root->right,k);
    }
    int KthSmallestInBST(node *root,int k){
        KthSmallestInBSTUtil(root,k);
        return kth_smallest;
    }

    bool CheckIfGivenTreeIsBSTUtil(node *root,pair<long long int,long long int>range){
        if(root==NULL) return 1;
        if(root->val>=range.second or root->val<=range.first) return 0;
        return (CheckIfGivenTreeIsBSTUtil(root->left,{range.first,root->val}) and CheckIfGivenTreeIsBSTUtil(root->right,{root->val,range.second}));
    }

    bool CheckIfGivenTreeIsBST(node *root){
